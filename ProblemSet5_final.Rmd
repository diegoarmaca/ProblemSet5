...
top4_crimes_19_to_23 <- 
  top4_crimes_19_to_23 %>% 
  mutate(over_21 = if_else(days_to_21 < 0, 0, 1)) 
...
stats_table <- select(stats_table, stats, days_to_21, over_21, assault_r, rape_r, murder_r, arson_r)
colnames(stats_table) <- c("stats","days_to_21 ","over_21","assault","rape","murder","arson")
...
lm(assault_r ~ days_to_21 + over_21, data = top4_crimes_19_to_23) %>% 
  tidy()
...
lm(rape_r ~ days_to_21 + over_21, data = top4_crimes_19_to_23) %>% 
  tidy()
...
lm(murder_r ~ days_to_21 + over_21, data = top4_crimes_19_to_23) %>% 
  tidy()
...
lm(arson_r ~ days_to_21 + over_21, data = top4_crimes_19_to_23) %>% 
  tidy()
